...
// --------- Helpers ---------
async function ensurePaymentTypeExists(paymentTypeId) {
...
router.post("/", (0, celebrate_1.celebrate)({
    [celebrate_1.Segments.BODY]: payment_validation_1.paymentBodySchema
}), async (req, res, next) => {
    try {
        ...
        await payment_service_1.PaymentService.ensurePaymentTypeExists(paymentTypeId);
        await payment_service_1.PaymentService.ensureNotDuplicatePayment({ date, paymentTypeId, description, amount });
...
router.get("/", (0, celebrate_1.celebrate)({
    [celebrate_1.Segments.QUERY]: payment_validation_1.paymentQueryListSchema
}), async (req, res, next) => {
    try {
...
router.get("/:id", (0, celebrate_1.celebrate)({
    [celebrate_1.Segments.PARAMS]: celebrate_1.Joi.object({
        id: celebrate_1.Joi.number().integer().positive().required()
    })
}), async (req, res, next) => {
    try {
...
router.put("/:id", (0, celebrate_1.celebrate)({
    [celebrate_1.Segments.PARAMS]: celebrate_1.Joi.object({
        id: celebrate_1.Joi.number().integer().positive().required()
    }),
    [celebrate_1.Segments.BODY]: payment_validation_1.paymentBodySchema
    ...
    try {
        ...
        await payment_service_1.PaymentService.ensurePaymentTypeExists(paymentTypeId);
        await payment_service_1.PaymentService.ensureNotDuplicatePayment({
            date,
            paymentTypeId,
            description,
            amount,
